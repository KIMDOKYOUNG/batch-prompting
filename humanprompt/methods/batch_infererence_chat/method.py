...
class BatchInferenceChatMethod(PromptChatMethod):
    def __init__(self, **kwargs: Any):
        super().__init__(**kwargs)

    def run(
        self,
        x: List[Union[str, Dict]],
        **kwargs: Any
    ) -> Union[str, List[str]]:
        client = openai.Client()
        verbose = kwargs.get("verbose", False)
        messages = PromptChatBuilder.build_chat_batch_prompt(
            x=x,
        ...
        return y, messages[-1]['content'], response
