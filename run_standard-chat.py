...
def run_experiment(
    ...
    Question_list, Answer_list, gold_list = [], [], []
    for idx, data_item in enumerate(dataset):
        if data_item.get('id', None) is None:
            data_item['id'] = idx
        if use_cache and \
                os.path.exists(os.path.join(tmp_save_dir, f"{idx}_{data_item['id']}.json")):
            ...
            while True:
                try:
                    ...
                    prediction, Question, Answer = method.run(
                        x=data_item,
                        verbose=verbose
                    )
            ...
            gold_list.append(str(data_item["answer"]))
    ...
    print(gold_list)
    f = open(os.path.join(save_dir, f"Question_{exp_name}.txt"), 'w')
    all_Question = '\n<d>\n'.join(Question_list)
    f.write(all_Question)
    f.close()
    f = open(os.path.join(save_dir, f"Answer_{exp_name}.txt"), 'w')
    all_Answer = '\n<d>\n'.join(Answer_list)
    f.write(all_Answer)
    f.close()
    f = open(os.path.join(save_dir, f"gold_{exp_name}.txt"), 'w')
    all_gold = '\n<d>\n'.join(gold_list)
    f.write(all_gold)
    f.close()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--exp_name", type=str, default="cot-rte-chat", help="Experiment name.")
    ...
    parser.add_argument("--verbose", type=bool, default=True, help="Whether to print verbose information.")
