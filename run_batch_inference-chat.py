...
def run_experiment(
        ...
        if use_cache \
            ...
            while True:
                try:
                    ...
                    batch_prediction, Question, Answer = method.run(
                        x=batch_data_items,
            ...
            gold_list.append('\n'.join([str(data_item['answer']) for data_item in batch_data_items]))
    ...
    f.close()
    f = open(os.path.join(save_dir, f"pred_{exp_name}.txt"), 'w')
    all_Answer = '\n<d>\n'.join(Answer_list)
    f.write(all_Answer)
    f.close()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--exp_name", type=str, default="batch_inference-rte-chat", help="Experiment name.")
